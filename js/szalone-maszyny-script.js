{
    ...
    const app = {
        initData(){
            this.data = dataSource.szaloneMaszyny;
            const elements = {};

            elements.intro = document.querySelector('.svg-intro');
            elements.game = document.querySelector('.svg-game');
            elements.bubbles = document.querySelector('.svg-upper-bubbles-bezjaj');
            elements.bezjaj = document.querySelector('.svg-bezjaj');
            elements.zjaj = document.querySelector('.svg-zjaj');
            elements.bu = document.querySelector('.svg-bubu');

            for (let element in elements){
                for (let svg in this.data.svgFiles){
                    if(element == svg){
                        elements[element].innerHTML = this.data.svgFiles[svg];
                    }
                }
            }
        ...
        animateIntro(){
            ...
            //check if its mobile- then play without scroll trigger ???
            //SCROLL-TRIGGERED INTR
        ...
        animateGame(){
                ...
                //playing in background game animations
                ...
                //LETS GO GAMING after scroll if desktop
                ...
                let tlStartGame = gsap.timeline({scrollTrigger: scroll });
            ...
            function letsPlay(){
                ...
                Draggable.create(virus, {
                    ...
                    onDrag: function(){   
                        ...
                        //smoke detectors and smokes animations
                        ...
                        //loosing viruses arms in lamps area
            ...
            //media queries
            //bez-jaj falling bubbles mediaquery and transform origin for bez-jaj scaling
            const screen = {
                small: window.matchMedia( '(max-width: 400px)' ),
                medium: window.matchMedia( '(min-width: 760px) and (max-width: 1300px)' ),
                // large : window.matchMedia( '(max-width: 1300px)' )
            };

            let tObezjaj = "36% 48.5%";
            let xFromZjaj = "-40vw";
            let xToZjaj = "30vw"
            // miedy 1300 a 760 ma miec 68.5
            if(screen.small.matches){
                tObezjaj = "36% 49%";
                xFromZjaj = "-60vw";
                xToZjaj = "20vw";
            }else if(screen.medium.matches){
                tObezjaj = "36% 49.05%";
            }
            ...
            function gameOver(){
                ...
                gameOverBackground.addEventListener("animationend", function() {
                    ...
                    let tlBezjaj = gsap.timeline({onComplete: app.gameEnd, onCompleteParams: ["game over.", "lost"]});
                    tlBezjaj.set('.svg-upper-bubbles-bezjaj', {x: "-50%", left: "50%", bottom: 0, width: "50vw"})
                        .from('.svg-upper-bubbles-bezjaj g', 0.7, { y: '-100vh', stagger: 0.2,  ease: Bounce.easeOut})
                        .to('.svg-upper-bubbles-bezjaj g', 0.7, { visibility: "visible", stagger: 0.2}, "<")
                        ...
                        .to('.svg-bezjaj', 4, { scale: 65, ease: Power1.easeIn, transformOrigin: tObezjaj })
            ...
            function winnerIs(score){
                ...
                tlZjaj.to('.blur', 0.5, {filter:"blur(3px)"})
                    .set('.svg-zjaj', {scale: 1 , height: "100vh", x: xFromZjaj})
                    .to('.zjaj', 0.3, {y: "-1%", repeat: -1, yoyo: true})
                    .to('.svg-zjaj', 6, {x: xToZjaj, ease: Power1.easeOut})
                    ...
                    .to('.svg-zjaj', 4 ,{x: "100vw", ease: Power1.easeIn, onComplete: app.gameEnd, onCompleteParams: [gamerTime]})
                    .to('.dymki-zjaj', 1, {scale: 0, transformOrigin: "50% 50%"}, "-=1.5");
            }
        },

        gameEnd(caption, lost) {
